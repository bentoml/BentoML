...
def test_save_load(local_yatai_container_url, example_bento_service_class):
    example_bento_service_class = bentoml.ver(major=2, minor=1)(
        example_bento_service_class
    )

    yc = get_yatai_client(local_yatai_container_url)
    test_model = TestModel()
    svc = example_bento_service_class()
    svc.pack('model', test_model)

    saved_path = svc.save(yatai_url=local_yatai_container_url)
    assert saved_path
...
def test_push(local_yatai_container_url, example_bento_service_class):
...
def test_pull(local_yatai_container_url, example_bento_service_class):
...
def test_push_with_labels(local_yatai_container_url, example_bento_service_class):
    example_bento_service_class = bentoml.ver(major=2, minor=2)(
        example_bento_service_class
    )
    ...
    print("GETTTTTING REMOTE")
    print(type(remote_bento_pb))
    print(remote_bento_pb)
...
def test_pull_with_labels(local_yatai_container_url, example_bento_service_class):
    example_bento_service_class = bentoml.ver(major=2, minor=3)(
        example_bento_service_class
    )
    ...
    svc.pack('model', test_model)
    saved_path = svc.save(
        yatai_url=local_yatai_container_url, labels={'foo': 'bar', 'abc': '123'}
    )
...
def test_get(local_yatai_container_url, example_bento_service_class):
    ...
    svc = example_bento_service_class()
    svc.pack('model', test_model)
    svc.save(yatai_url=local_yatai_container_url)
...
def test_list(local_yatai_container_url, example_bento_service_class):
...
def test_load(local_yatai_container_url, example_bento_service_class):
    example_bento_service_class = bentoml.ver(major=2, minor=5)(
        example_bento_service_class
    )
    yc = get_yatai_client(local_yatai_container_url)
