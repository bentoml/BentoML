...
from bentoml._internal.spark import _load_bento  # ignore errors?
from bentoml._internal.spark import _get_process
...
def test_distribute_bento(spark_session: SparkSession) -> None:
    assert (
        _distribute_bento(spark_session, get_bento(bento_tag))
...
def test_get_process() -> None:
    ...
    assert callable(
        _get_process(bento_tag, api_name)
