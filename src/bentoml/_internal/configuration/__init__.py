...
def config_file_from_env() -> str | None:
    if CONFIG_ENV_VAR in os.environ:
        # User local config file for customizing bentoml
        bentoml_config_file = expand_env_var(os.environ.get(CONFIG_ENV_VAR, ""))
        ...
        return bentoml_config_file
    return None


def config_values_from_env() -> str | None:
...
def load_default_config():
    ...
    BentoMLContainer.config.set(
        BentoMLConfiguration(
            override_config_file=config_file_from_env(),
            override_config_values=config_values_from_env(),
        ).to_dict()
    )
