fast.ai
=======

.. admonition:: About this page

    This is an API reference for using FastAI in BentoML. Please refer to :doc:`fastai
    guide </frameworks/fastai>` for more information about how to use FastAI in BentoML.

.. currentmodule:: bentoml.fastai

.. autofunction:: bentoml.fastai.save_model

.. autofunction:: bentoml.fastai.load_model

.. autofunction:: bentoml.fastai.get
