...
class _NestedSequence(Protocol[_T_co]):
    ...
    *e.g.* ``def func(a: _NestedSequnce[T]) -> T: ...``.
    ...
    def __len__(self, /) -> int:
        """Implement ``len(self)``."""
    @overload
    def __getitem__(self, index: int, /) -> Union[_T_co, _NestedSequence[_T_co]]: ...
    @overload
...
def frombytes(
    mode: _Mode, size: tuple[int, int], data, decoder_name: str = ..., *args
) -> Image: ...
def frombuffer(
    mode: _Mode, size: tuple[int, int], data, decoder_name: str = ..., *args
) -> Image: ...

# obj can be array interface
# if obj is not contiguous, then convert to bytes then put it into a stream via `frombuffer()`
def fromarray(obj: ArrayLike, mode: _Mode | None = ...) -> Image: ...
