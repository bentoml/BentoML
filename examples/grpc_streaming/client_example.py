...
async def run_client():
    ...
    target_address = "localhost:50051" # Default gRPC port for BentoML, adjust if necessary
    ...
    async with grpc.aio.insecure_channel(target_address) as channel:
        ...
        try:
            ...
            async for response in stub.Chat(request_message):
                print(f"Server says: {response.text} (ID: {response.message_id}, TS: {response.timestamp})")
