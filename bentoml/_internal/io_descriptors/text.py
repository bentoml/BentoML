...
class Text(IODescriptor[str], proto_fields=["string_value", "raw_value"]):
    ...
    async def from_grpc_request(self, request: _service_pb2.Request, context) -> str:
        return str(request.input.string_value)

    async def to_grpc_response(self, obj: str, context) -> _service_pb2.Response:
        return _service_pb2.Response(output=service_pb2.Value(string_value=obj))
