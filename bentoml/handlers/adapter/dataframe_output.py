...
class DataframeOutput(BaseOutputAdapter):
    ...
    def __init__(self, output_orient='records'):
        super(DataframeOutput, self).__init__()
    ...
    def to_batch_response(
        self, result_conc, slices=None, requests: Iterable[SimpleRequest] = None,
    ) -> Iterable[SimpleResponse]:
        # TODO(bojiang): header content_type
        fallback = lambda: SimpleResponse(400, None, "bad request")

        if slices is None:
            results = [r for r in result_conc]
        else:
            results = [result_conc[s] if s else self.BadResult for s in slices]

        responses = [None] * len(results)
        for i, result in enumerate(results):
            if result is self.BadResult:
