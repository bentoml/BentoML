Pickable Model
==============

This is an API reference the ``bentoml.picklable_model`` module, which can be used for
custom Python-based ML models in BentoML. To learn more, visit :doc:`Pickable Model
</frameworks/pickable>`.

.. currentmodule:: bentoml.picklable_model

.. autofunction:: bentoml.picklable_model.save_model

.. autofunction:: bentoml.picklable_model.load_model

.. autofunction:: bentoml.picklable_model.get
