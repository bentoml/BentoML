...
def indices_fast(
    index: np.ndarray, labels: np.ndarray, keys: list, sorted_labels: list[np.ndarray]
) -> dict: ...
def generate_slices(
    labels: np.ndarray, ngroups: int
) -> tuple[np.ndarray, np.ndarray]: ...
def count_level_2d(
    mask: np.ndarray, labels: np.ndarray, max_bin: int, axis: int
) -> np.ndarray: ...
def get_level_sorter(label: np.ndarray, starts: np.ndarray) -> np.ndarray: ...
def generate_bins_dt64(
    values: np.ndarray, binner: np.ndarray, closed: object = ..., hasnans: bool = ...
) -> np.ndarray: ...
