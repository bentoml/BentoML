...
let stubs = Bentoml_Grpc_V1alpha2_BentoServiceNIOClient(channel: channel)

// Form the request with the NDArray, if one was provided.
let ndarray: Bentoml_Grpc_V1alpha2_NDArray = .with {
  $0.shape = shape
  $0.floatValues = data
  $0.dtype = Bentoml_Grpc_V1alpha2_NDArray.DType.float
}

let request: Bentoml_Grpc_V1alpha2_Request = .with {
  $0.apiName = apiName
  $0.ndarray = ndarray
}
