...
class LocalBundleManagement:
    ...
    _bundle_store_path = _get_bundle_store_path()
    ...
    def list(self, name=None):
        ...
        names_path = os.path.abspath(self._bundle_store_path)
        bundles = []
    ...
    def get(self, tag):
        ...
        tag_path = os.path.join(self._bundle_store_path, name, version)
    ...
    def delete(self, name=None, tag=None, skip_confirmation=None):
        ...
        if tag:
            ...
            if not os.path.isdir(tag_path):
                return None
            if self.confirm(f"delete {tag}?"):
                self._logger.info(f"deleting {tag}")
                shutil.rmtree(tag_path)
            else:
                self._logger.info(f"skipping {tag}")
        elif name:
            name_path = os.path.join(self._bundle_store_path, name)
            if not os.path.isdir(name_path):
                return None
            if skip_confirmation:
                self._logger.info(f"deleting {name}")
            ...
            if skip_confirmation:
                # do we need to preserve the empty bundles directory?
                # shutil.rmtree(self._bundle_store_path)
                self._logger.info(f"deleting all bundles")
                names_path = os.path.abspath(self._bundle_store_path)
    ...
    def push(self, tag, yatai=None):
        if not yatai:
            # set default yatai server
            pass


    def pull(self, tag, yatai=None):
        if not yatai:
            # set default yatai server  
            pass
