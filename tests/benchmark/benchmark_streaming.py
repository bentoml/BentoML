...
async def benchmark_grpc_stream(
    host: str, port: int, payload_size: int, stream_length: int, iterations: int
) -> Dict[str, Any]:
    ...
    print(
        f"gRPC: Payload size: {payload_size} bytes, Stream length: {stream_length} messages, Iterations: {iterations}"
    )
        ...
        for i in range(iterations):
            ...
            try:
                async for response in client.call_stream(data=payload):
                    ...
                    if (
                        message_count >= stream_length
                    ):  # Ensure we don't stream indefinitely if server sends more
                        break
                ...
                if message_count == stream_length:
                    ...
                    if first_response_time:
                        first_response_latencies.append(
                    ...
                    print(
                        f"gRPC Iteration {i + 1}: Failed - Expected {stream_length} messages, got {message_count}"
                    )

            except grpc.aio.AioRpcError as e:
                print(
                    f"gRPC Iteration {i + 1}: Failed with gRPC error - {e.code()}: {e.details()}"
                )
    ...
    avg_first_response_latency = (
        statistics.mean(first_response_latencies) if first_response_latencies else -1
    )

    total_bytes_per_iteration = statistics.mean(total_bytes_streamed_list)
    throughput_mps = (
        successful_iterations / sum(timings) if sum(timings) > 0 else 0
    )  # Total successful messages / total time for successful
    throughput_bps = (
        sum(total_bytes_streamed_list) / sum(timings) if sum(timings) > 0 else 0
    )
    ...
    return {
        ...
        "throughput_msgs_per_s": throughput_mps
        * stream_length,  # average messages per second across successful streams
...
async def benchmark_rest_stream(
    ...
    print(
        f"REST: Payload size: {payload_size} bytes, Stream length: {stream_length} messages, Iterations: {iterations}"
    )
    ...
    async with httpx.AsyncClient(base_url=f"http://{host}:{port}") as client:
        ...
        try:
            async with client.stream(
                "POST", endpoint, json={"data": "warmup", "stream_length": 1}
            ) as response:
                async for _ in response.aiter_lines():
                    pass
        except httpx.RequestError as e:
            print(
                f"REST Warmup failed: {e}. Ensure REST server is running and endpoint '{endpoint}' exists."
            )
        ...
        for i in range(iterations):
            ...
            try:
                async with client.stream(
                    "POST", endpoint, json=json_payload_with_length, timeout=30.0
                ) as response:
                    if response.status_code != 200:
                        print(
                            f"REST Iteration {i + 1}: Failed - Status {response.status_code}, {await response.aread()}"
                        )
                        continue
                ...
                if (
                    message_count == stream_length
                ):  # Validate if server sent expected number of messages
                    timings.append(end_time - start_time)
                    if first_response_time:
                        first_response_latencies.append(
                            first_response_time - start_time
                    ...
                    print(
                        f"REST Iteration {i + 1}: Failed - Expected {stream_length} messages, got {message_count}"
                    )
...
# --- Main Execution ---
def display_results(
    scenario: Dict[str, Any], grpc_results: Dict[str, Any], rest_results: Dict[str, Any]
):
    print("\n--- Benchmark Scenario ---")
    ...
    if "error" in grpc_results:
        ...
        print(
            f"  Successful Iterations: {grpc_results['successful_iterations']}/{scenario['iterations']}"
        )
        print(
            f"  Avg. Stream Time: {grpc_results['avg_stream_time_s']:.4f} s (StdDev: {grpc_results['std_dev_stream_time_s']:.4f} s)"
        )
        print(
            f"  Avg. First Response Latency: {grpc_results['avg_first_response_latency_s']:.4f} s"
        )
        print(
            f"  Throughput (Streams/s): {grpc_results['throughput_streams_per_s']:.2f}"
        )
    ...
    if "error" in rest_results:
        ...
        print(
            f"  Successful Iterations: {rest_results['successful_iterations']}/{scenario['iterations']}"
        )
        print(
            f"  Avg. Stream Time: {rest_results['avg_stream_time_s']:.4f} s (StdDev: {rest_results['std_dev_stream_time_s']:.4f} s)"
        )
        print(
            f"  Avg. First Response Latency: {rest_results['avg_first_response_latency_s']:.4f} s"
        )
        print(
            f"  Throughput (Streams/s): {rest_results['throughput_streams_per_s']:.2f}"
        )
...
async def main():
    ...
    parser.add_argument(
        "--grpc_host", type=str, default=DEFAULT_HOST, help="gRPC server host."
    )
    parser.add_argument(
        "--grpc_port", type=int, default=DEFAULT_GRPC_PORT, help="gRPC server port."
    )
    parser.add_argument(
        "--rest_host", type=str, default=DEFAULT_HOST, help="REST server host."
    )
    parser.add_argument(
        "--rest_port", type=int, default=DEFAULT_REST_PORT, help="REST server port."
    )
    parser.add_argument(
        "--rest_endpoint", type=str, default="/stream", help="REST streaming endpoint."
    )
    parser.add_argument(
        "--iterations",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="10,100,500",
        help="Comma-separated stream lengths (number of messages).",
    )
    parser.add_argument(
        "--skip_grpc", action="store_true", help="Skip gRPC benchmarks."
    )
    parser.add_argument(
        "--skip_rest", action="store_true", help="Skip REST benchmarks."
    )
    ...
    print(
        "Important: Ensure both gRPC and REST servers are running and configured for streaming."
    )
    print(
        "The REST server must accept POST requests at the specified endpoint, expecting a JSON payload like"
    )
    print(
        "{'data': 'your_payload', 'stream_length': number_of_messages} and stream back line-delimited text responses."
    )
