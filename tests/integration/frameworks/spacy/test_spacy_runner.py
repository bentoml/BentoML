...
from tests.integration.frameworks.spacy.test_spacy_impl import (  # noqa
    spacy_model,
    test_json,
)
...
def test_spacy_runner_setup_run_batch(modelstore, spacy_model):
    ...
    res = runner._run_batch([test_json["text"]])
    for i in res:
...
def test_spacy_runner_setup_on_gpu(spacy_model, modelstore, dev, backend):
    tag = bentoml.spacy.save(MODEL_NAME, spacy_model, model_store=modelstore)
