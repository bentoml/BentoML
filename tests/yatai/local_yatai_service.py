...
def local_yatai_service_container(image_tag, db_url=None, repo_base_url=None):
    ensure_docker_available_or_raise()
    docker_client = docker.from_env()
    yatai_docker_image_tag = f'bentoml/yatai-service:e2e-test-{image_tag}'

    try:
        docker_client.images.get(yatai_docker_image_tag)
    except docker.errors.ImageNotFound:
        # Note: When set both `custom_context` and `fileobj`, docker api will not use
        # the `path` provide... docker/api/build.py L138. The solution is create an
        # actual Dockerfile along with path, instead of fileobj and custom_context.
