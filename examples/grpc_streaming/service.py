...
from bentoml.io import (
    Text,  # For any potential REST/HTTP endpoints, not used by gRPC directly
)


# Servicer implementation
class SimpleStreamingServicerImpl(
    example_service_pb2_grpc.SimpleStreamingServiceServicer
):
    async def Chat(self, request: example_service_pb2.ChatMessage, context):
        print(
            f"Received chat message from client: '{request.text}' (ID: {request.message_id})"
        )
        ...
        for i in range(5):  # Stream back 5 messages
            ...
            yield example_service_pb2.ChatMessage(
                message_id=message_id, text=response_text, timestamp=timestamp
            )
...
svc.mount_grpc_servicer(
    ...
    service_names=[
        example_service_pb2.DESCRIPTOR.services_by_name[
