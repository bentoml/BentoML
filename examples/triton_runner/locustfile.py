from __future__ import annotations

from locust import task
from locust import constant
from locust import HttpUser


class TritonTorchscriptMnistUser(HttpUser):
    wait_time = constant(0)
    filename = "./data/8.png"

    @task
    def triton_torchscript_mnist_infer(self):
        self.client.post(
            "/triton_torchscript_mnist_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/png"))],
        )


class TritonOnnxMnistUser(HttpUser):
    wait_time = constant(0)
    filename = "./data/8.png"

    @task
    def triton_onnx_mnist_infer(self):
        self.client.post(
            "/triton_onnx_mnist_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/png"))],
        )


class TritonTensorflowMnistUser(HttpUser):
    wait_time = constant(0)
    filename = "./data/8.png"

    @task
    def triton_tensorflow_mnist_infer(self):
        self.client.post(
            "/triton_tensorflow_mnist_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/png"))],
        )


class BentoTorchscriptMnistUser(HttpUser):
    wait_time = constant(0)
    filename = "./data/8.png"

    @task
    def bentoml_torchscript_mnist_infer(self):
        self.client.post(
            "/bentoml_torchscript_mnist_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/png"))],
        )


class BentoOnnxMnistUser(HttpUser):
    wait_time = constant(0)
    filename = "./data/8.png"

    @task
    def bentoml_onnx_mnist_infer(self):
        self.client.post(
            "/bentoml_onnx_mnist_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/png"))],
        )


class BentoTensorflowMnistUser(HttpUser):
    wait_time = constant(0)
    filename = "./data/8.png"

    @task
    def bentoml_tensorflow_mnist_infer(self):
        self.client.post(
            "/bentoml_tensorflow_mnist_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/png"))],
        )


class TritonTorchscriptYolov5User(HttpUser):
    wait_time = constant(0)
    filename = "./data/zidane.jpg"

    @task
    def triton_torchscript_yolov5_infer(self):
        self.client.post(
            "/triton_torchscript_yolov5_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/png"))],
        )


class TritonOnnxYolov5User(HttpUser):
    wait_time = constant(0)
    filename = "./data/zidane.jpg"

    @task
    def triton_onnx_yolov5_infer(self):
        self.client.post(
            "/triton_onnx_yolov5_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/jpeg"))],
        )


class TritonTensorflowYolov5User(HttpUser):
    wait_time = constant(0)
    filename = "./data/zidane.jpg"

    @task
    def triton_tensorflow_yolov5_infer(self):
        self.client.post(
            "/triton_tensorflow_yolov5_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/jpeg"))],
        )


class BentoTorchscriptYolov5User(HttpUser):
    wait_time = constant(0)
    filename = "./data/zidane.jpg"

    @task
    def bentoml_torchscript_yolov5_infer(self):
        self.client.post(
            "/bentoml_torchscript_yolov5_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/jpeg"))],
        )


class BentoOnnxYolov5User(HttpUser):
    wait_time = constant(0)
    filename = "./data/zidane.jpg"

    @task
    def bentoml_onnx_yolov5_infer(self):
        self.client.post(
            "/bentoml_onnx_yolov5_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/jpeg"))],
        )


class BentoTensorflowYolov5User(HttpUser):
    wait_time = constant(0)
    filename = "./data/zidane.jpg"

    @task
    def bentoml_tensorflow_yolov5_infer(self):
        self.client.post(
            "/bentoml_tensorflow_yolov5_infer",
            files=[("file", (self.filename, open(self.filename, "rb"), "image/jpeg"))],
        )
