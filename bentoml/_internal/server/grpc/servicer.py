...
def create_bento_servicer(service: Service) -> services.BentoServiceServicer:
    ...
    class BentoServiceImpl(services.BentoServiceServicer):
        ...
        async def Call(  # type: ignore (no async types) # pylint: disable=invalid-overridden-method
            ...
            try:
                _use_internal_bytes_contents = False
                # map by default is optional ,and hence we need to check
                # whether multipart is an empty dict or not.
                if len(request.multipart) != 0:
                    io_proto = request.multipart
                elif request.HasField(api.input.accepted_proto_fields):
                    io_proto = getattr(request, api.input.accepted_proto_fields)
                elif request.HasField("_internal_bytes_contents"):
                    io_proto = request._internal_bytes_contents
                    _use_internal_bytes_contents = True
                else:
                    raise InvalidArgument(
                        f"Neither '{api.input.accepted_proto_fields}' or '_internal_bytes_contents' field is found in the request message.",
                    ) from None
                input_ = await api.input.from_proto(
                    io_proto, _use_internal_bytes_contents=_use_internal_bytes_contents
                )
                ...
                response = pb.Response(**{api.output.accepted_proto_fields: protos})
