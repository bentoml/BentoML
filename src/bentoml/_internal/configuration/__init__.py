...
# not inside bazel runfiles.
__version__ = "0.1.0.dirty"
__version_tuple__ = (0, 1, 0, "dirty")
if not run_in_bazel():
    try:
        ...
        try:
            # This is the Git root relative from this file.
            _git_root = str(
                Path(__file__).parent.parent.parent.parent.parent.resolve().absolute()
            )
            __version__ = get_version(
                root=_git_root, version_scheme="post-release", write_to=None
            )
        except LookupError as e:
            raise BentoMLException(
                f"Failed to parse version from git source: {e}"
            ) from None
