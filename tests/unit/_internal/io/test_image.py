...
@pytest.mark.asyncio
async def test_from_proto(use_internal_bytes_contents: bool, img_file: str):
    with open(img_file, "rb") as f:
        content = f.read()
    if use_internal_bytes_contents:
        res = await Image(mime_type="image/bmp").from_proto(
        ...
        res = await Image(mime_type="image/bmp").from_proto(
            pb.Part(file=pb.File(kind=pb.File.FILE_TYPE_BMP, content=content)),
