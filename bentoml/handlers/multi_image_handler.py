...
def _import_imageio_imread():
    try:
        ...
        raise MissingDependencyException(
            "imageio package is required to use MultiImageHandler"
        )
...
class MultiImageHandler(BentoHandler):
    ...
    Raises:
        ImportError: imageio package is required to use MultiImageHandler
    """
    ...
    def __init__(
        ...
        **base_kwargs,
        ...
        super(MultiImageHandler, self).__init__(**base_kwargs)
    ...
    def handle_batch_request(
        ...
        for i, req in enumerate(requests):
            ...
            if len(self.input_names) == 1 and len(request.files) == 1:
                # Ignore multipart form input name when MultiImageHandler is intended to
                # accept only one image file at a time
        ...
        results = func(input_datas)
        json_outputs = [api_func_result_to_json(result) for result in results]
        return [SimpleResponse(200, None, json_output) for json_output in json_outputs]
    ...
    def handle_request(self, request, func):
        ...
        """
        if len(self.input_names) == 1 and len(request.files) == 1:
            # Ignore multipart form input name when MultiImageHandler is intended
        ...
        if input_files:
            ...
            if data:
                ...
                raise BadInput(
                    "BentoML#MultiImageHandler unexpected HTTP request format"
                )
