    ...
    async with create_channel(host) as channel:
        ...
        with pytest.raises(aio.AioRpcError):
            ...
            await async_client_call(
                ...
                channel=channel,
                data={
                    "_internal_bytes_contents": np.array([1, 2, 3, 4]).ravel().tobytes()
                },
                assert_code=grpc.StatusCode.FAILED_PRECONDITION,
            )
            await async_client_call(
                "double_ndarray",
                channel=channel,
...
async def test_json(host: str):
    async with create_channel(host) as channel:
        ...
        await async_client_call(
            ...
            channel=channel,
            data={
                "_internal_bytes_contents": b'{"request_id": "123", "iris_features": {"sepal_len":2.34,"sepal_width":1.58, "petal_len":6.52, "petal_width":3.23}}'
            },
        ...
        with pytest.raises(aio.AioRpcError):
            await async_client_call(
                "echo_json",
                channel=channel,
                data={"_internal_bytes_contents": b"\n?xfa"},
                assert_code=grpc.StatusCode.INVALID_ARGUMENT,
            )
            await async_client_call(
                "echo_json",
...
async def test_file(host: str, bin_file: str):
    ...
    async with create_channel(host) as channel:
        await async_client_call(
            "predict_file",
            channel=channel,
            data={"_internal_bytes_contents": fb},
