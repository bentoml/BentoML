XGBoost
=======

.. admonition:: About this page

    This is an API reference for XGBoost in BentoML. Please refer to the :ref:`XGBoost
    guide <frameworks/xgboost:XGBoost>` for more information about how to use XGBoost in
    BentoML.

.. currentmodule:: bentoml.xgboost

.. autofunction:: bentoml.xgboost.save_model

.. autofunction:: bentoml.xgboost.load_model

.. autofunction:: bentoml.xgboost.get
