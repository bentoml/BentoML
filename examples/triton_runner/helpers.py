...
def safe_create_bentomodel(f: t.Callable[..., t.Any] | None = None):
    def decorator(
        save_model_func: t.Callable[P, bentoml.Model]
    ) -> t.Callable[P, t.Any]:
        fname = save_model_func.__name__.split("_", maxsplit=1)
        if len(fname) != 2:
            raise ValueError(
                "Make sure the function name is in the format: <model_type>_<framework>"
            )
        framework = fname[0]

        model_name = save_model_func.__name__.replace("_", "-")

        @functools.wraps(save_model_func)
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> None:
            override = kwargs.pop("override", False)
            kwargs["bento_model_name"] = model_name
            try:
                m = bentoml.models.get(model_name)
                if override:
                    print(
                    ...
                    print(
                        "Saved model:",
                        save_model_func(*args, **kwargs),
                    )
                else:
                    print(f"Model for {framework} already exists in local store:", m)
            except bentoml.exceptions.NotFound:
                model = save_model_func(*args, **kwargs)
                print("Saved model:", model)
            except Exception:  # pylint: disable=broad-except
                print("Failed to save model:", traceback.format_exc())
                raise
