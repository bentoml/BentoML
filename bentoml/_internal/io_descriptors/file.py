...
class File(IODescriptor[FileType]):
    ...
    async def to_proto(self, obj: FileType) -> pb.File:
        ...
        if self._mime_type.startswith("multipart"):
            raise BadInput(
                f"'{repr(self)}' has 'mime_type={self._mime_type}', which is not allowed to use with 'to_proto'. Use Multipart() to handle multiple outputs IO."
            ) from None
...
class BytesIOFile(File):
    ...
    async def from_proto(
        ...
        *,
        ...
        if self._mime_type.startswith("multipart"):
            raise BadInput(
                f"'{repr(self)}' has 'mime_type={self._mime_type}', which is not allowed to use with 'from_proto'. Use Multipart() to handle multiple outputs IO."
            ) from None
