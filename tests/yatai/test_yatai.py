...
def test_save_load(yatai_server_container, example_bento_service_class):
    ...
    saved_path = yatai.save(svc, yatai_url=yatai_server_container)
    assert saved_path

    bento_service = yatai.load(
        f'{svc.name}:{svc.version}',
        yatai_url=yatai_server_container
    )
...
def test_push(yatai_server_container, example_bento_service_class):
    ...
    svc = example_bento_service_class()
    svc.pack('model', test_model)
    saved_path = yatai.save(svc)

    pushed_path = yatai.push(
        f'{svc.name}:{svc.version}',
        yatai_url=yatai_server_container
    )
...
def test_get(yatai_server_container, example_bento_service_class):
    ...
    svc = example_bento_service_class()
    svc.pack('model', test_model)
    yatai.save(svc, yatai_url=yatai_server_container)
    svc_metadata = yatai.get(
        f'{svc.name}:{svc.version}',
        yatai_url=yatai_server_container
    )
...
def test_list(yatai_server_container, example_bento_service_class):
    ...
    bentos = yatai.list(bento_name=svc.name, yatai_url=yatai_server_container)
    assert len(bentos) == 5
