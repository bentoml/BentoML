...
const DeploymentInfo = ({ deployment }) => {
  ...
  const renderValue = rowIndex => {
    ...
    switch (fieldName) {
      case "Created at":
        value = displayTimeInFromNowFormat(
          Number(deployment.created_at.seconds)
        );
        break;
      case "Updated at":
        value = displayTimeInFromNowFormat(
      ...
      case "Endpoint":
        ...
        if (state.state == "RUNNING" && state.info_json) {
          ...
          value = <pre>{infoJson.endpoints.join("\n")}</pre>;
        }
        break;
    }
