...
def host(image_name):
    ...
    with run_api_server_docker_container(image_name, timeout=500) as host_name:
        yield host_name


@pytest.mark.asyncio
async def test_pytorch_lightning_with_docker(host_name):
    await pytest.assert_request(
        "POST",
        f"http://{host_name}/predict",
