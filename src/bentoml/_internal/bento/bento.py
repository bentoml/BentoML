...
class Bento(StoreItem):
    ...
    def doc(self) -> str:
        ...
        if info.size > 3 * 1024 * 1024:  # 3MB, Limit of bentocloud api is 4MB, leave 1MB for other metadata
            logger.warning("Bento README is too large (%d bytes > 3MB), skipping upload...", info.size)
            return ""
