...
__bentoml_commandgroup() {
  ...
  case "$words[1]" in
    (version)
      _arguments $(__bentoml_arguments) && ret=0
      ;;
    (build)
      _arguments $(__bentoml_arguments) -C \
        $opts_help \
    ...
    (containerize)
      ...
      _arguments $(__bentoml_arguments) -C \
        ...
        "($help)--enable-features=[Enable additional BentoML features]:features:(tracing grpc tracing.zipking tracing.jaeger tracing.otlp)" \
    ...
    (serve|serve-http)
      _arguments $(__bentoml_arguments) -C \
        ...
        "($help)--ssl-certfile[SSL certificate file]:path to certificate file (str):_files -/" \
        "($help)--ssl-keyfile[SSL key file]:path to key file (str):_files -/" \
    ...
    (serve-grpc)
      _arguments $(__bentoml_arguments) -C \
        $opts_help \
        "($help)--production[Run BentoServer in production mode]" \
        "($help)--port[Port to listen on for REST API server, (default: 50051)]:port (int): " \
        "($help)--host[The host to bind for the gRPC server (defaults: 0.0.0.0)]: host (0.0.0.0): " \
        "($help)--api-workers[Specify the number of API server workers to start. Default to number of available CPU cores in production mode]:num workers (int): " \
        "($help)--backlog[Number of pending connection, (default: 2048)]:backlog (int): " \
        "($help)--reload[Reload Service when code changes detected, this is only available in development mode]" \
        "($help)--enable-reflection[Whether to enable reflection or not]" \
        "($help)--max-concurrent-streams[Maximum number of concurrent incoming streams to allow on a HTTP2 connection.]: number (int): " \
        "($help)--working-dir[When loading from source code, specify the directory to find the Service instance (default: .)]:working dir (str):_files -/" \
        "($help)--ssl-certfile[SSL certificate file]:path to certificate file (str):_files -/" \
        "($help)--ssl-keyfile[SSL key file]:path to key file (str):_files -/" \
        "($help -):bento tag:{_alternative ':path:_files' __bentoml_complete_bento_tag}" && ret=0
